...
#if defined(NDEBUG)
# error "DigitalCFA cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc689ce8c9883ae00285434aa5b2a694fd2a41bd22eccb676a79a37a7dbdeddc5");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // DigitalCFA: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "DigitalCFA Signed Message:\n";
...
{
    // DigitalCFA: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // DigitalCFA: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // DigitalCFA: 1 day
static const int64 nTargetSpacing = 5 * 60; // DigitalCFA: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // DigitalCFA: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // DigitalCFA: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xc93205e7c5cc2796e191bd3055f0f8a16b0519a01d4f3833bb64098967c74342");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "13/Nov/2015 Bataclan: Fluctuat nec mergitur, deadly attack from terrorists 130 deads and 413 wounded";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04c69874810727c064fb3fc5463ea86f6b0073558317b3971d87129296cabec2856887fb764084f1880df7ddf208ad4274cd3e79422cb1d92e6bcc8f2bd8d82acb") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1528127168;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086652462;

        if (fTestNet)
        {
            block.nTime    = 1528127087;
            block.nNonce   = 386414263;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xb1d71aa441681bbd413307f2f4c77d317d69369c5407112409f189243a3141e1"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf1, 0xc2, 0xb0, 0xd3 }; // DigitalCFA: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DigitalCFAMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DigitalCFAMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DigitalCFAMiner(CWallet *pwallet)
{
    printf("DigitalCFAMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("digitalcfa-miner");
    ...
    try { loop {
        ...
        printf("Running DigitalCFAMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DigitalCFAMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DigitalCFAMiner, pwallet));
}
